...
describe Board do
  ...
  describe '#placing ' do 
    ...
    it 'will not place a ship beyond the board' do
      expect{subject.place ship,subject.size+1,subject.size+1}.to raise_error('Outside the board')
    end

    it 'should increase the count of ships' do 
      subject.place(ship,1,1)
      expect(subject.n_of_X).to eq 1
    end 

    it 'cannot place a ship on another one' do 
      subject.place(ship,1,1)
      expect{subject.place(ship,1,1)}.to raise_error 'Those coordinates are already used!'
    end 
  end 
